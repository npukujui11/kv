...
bool LRUCache<K, V>::get(const K& key, V& value) {
    ...
    /*
     * 惰性删除策略
     * 在 get() 函数中：当从哈希表中找到某个键值时，我们不仅要返回数据，还要检查该数据是否过期。
     * 过期处理：如果该数据过期，就将其从双向链表和哈希表中移除。
     * 这就是“惰性删除”的核心，它只在访问时才删除过期的数据，而不在其他时间做定期清理。
     * 未过期数据的处理：如果数据未过期，将该节点移动到双向链表的头部，表示它是最近使用的。
     */
